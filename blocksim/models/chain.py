...
class Chain:
    ...
    def __init__(self, env, node, consensus, genesis, db):
        ...
        #self.db.put(f'block:{genesis.header.number}', [genesis.header.hash,genesis.header.hash])
        #self.db.put(f'block_xor:{genesis.header.xorhash}', genesis.header.xorhash)
        self.db.put(genesis.header.hash, genesis)
        self._head_hash = genesis.header.hash
        
        #self._head_xorhash = genesis.header.xorhash
        #print(self._head_xorhash)
        self.parent_queue = {}
    ...
    def add_child(self, child):
        ...
        #for i in range(0, len(existing_xor), 32):
            #existing_xorhashes.append(existing_xor[i: i + 32])
        #if k not in existing_xorhashes:
            #self.db.put('child_xor:' + k, existing_xor + k)
    ...
    def add_block(self, block):
        ...
        if block.header.prevhash == self._head_hash:
        #if block.header.prevhash == self._head_hash and block.header.xorhash == self._head_xorhash:
            print(
            ...
            #self._head_xorhash = block.header.xorhash
        ...
        # Block has no parent yet. An Orphan block
            ...
            if block.header.prevhash not in self.parent_queue:
                ...
                #self.parent_queue[block.header.xorhash] = []
            self.parent_queue[block.header.prevhash].append(block)
            #self.parent_queue[block.header.xorhash].append(block)
            print(
